...
function SpriteHandler.new()
    ...
    function self:setXML(filePath)
        ...
        for name, x, y, width, height, frameX, frameY, frameWidth, frameHeight in data:gmatch('<SubTexture name="([^"]+)" x="(%d+)" y="(%d+)" width="(%d+)" height="(%d+)" frameX="([%-]?%d+)" frameY="([%-]?%d+)" frameWidth="(%d+)" frameHeight="(%d+)"') do
            ...
            table.insert(self.animations[animationName], {
                ...
                width = tonumber(frameWidth),
                height = tonumber(frameHeight),
                offsetX = tonumber(frameX),
                offsetY = tonumber(frameY)
            })
        end
    end
    ...
    function self:draw(x, y, rotation)
